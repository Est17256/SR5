...
class Bitmap(object):
    ...
    def load3(self, filename, tra, sca, texture):
        mod = Obj(filename)
        luz = V3(0, 0, 1)
        for car in mod.cars:
            vcount = len(car)
            if vcount == 3:
                ...
                if not texture:
                    ...
                    if mon < 0:
                        continue
                    self.triangleS(a, b, c, color(random(0, 255), random(0, 255), random(0, 255)))
                else:
                    mon = round(255 * inte)
                    t1 = car [0][1] - 1
                    t2 = car [1][1] - 1
...
r.load3('cubo.obj', tra=(3, 2, 0), sca=(100, 100, 100),texture=t)
r.glFinish('SR.bmp')
